...
obs = np.linspace(59630.320217,59630.320217+(10*365.25),1000)
obs = obs.tolist()
obs = [obs]*7
# In[19]:
...
def flaring(B, length, dayinsec=86400,amplitude=1):   
    ...
    conjugate = [(x.conjugate()) for x in real]
    conjugate.extend(real)
    neglogF = [-x for x in logF]
    fulllogF = neglogF.extend(logF)
    plt.plot(conjugate,neglogF)
    plt.plot(real,logF)
    plt.title('real')
    plt.show()
    
    #print(combined)
    IFT = np.fft.ifft(conjugate)
    #IFT = np.real(IFT)
    #print(IFT[0:15])
    ...
    print(np.std(flareMag))
    print(len(flareMag[0:len(minutes)]),len(minutes))
    plt.plot(IFT)
    plt.title('IFT')
    plt.show()
    plt.plot(positive)
    ...
    plt.title('normalised')
    plt.show()
    plt.plot(minutes,flareMag[0:len(minutes)])
    plt.title('lightcurve')
    plt.show()
    return flareMag

flareone = flaring(-5, flareperiod, dayinsec=(86400),amplitude=0.3)
#flareone = flaring(-1, flareperiod, dayinsec=(86400*365.25),amplitude=0.3)
#flareone = flaring(-1, flareperiod, dayinsec=(86400*365.25*10),amplitude=0.3)
...
#     plt.show()
    ...
    plt.savefig('rMag'+str(objectmag[3])+'Period'+str(period)+'LS.png')
    plt.show()
...
#lombScargle(FrangeLoop[-1],objectmag=newlist[0])
